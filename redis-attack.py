...
def main():
    ...
    parser.add_argument("-lf2", "--linuxfile2", type=str, help="Crontab file used to attack linux(/var/spool/cron/root), default exp/linux/exp.crontab(must be modified before use)", default=os.path.join('exp','linux','exp.crontab'))
    parser.add_argument("-lf3", "--linuxfile3", type=str, help="SSH id_rsa.pub file used to attack linux(/root/.ssh/authorized_keys), use `ssh-keygen -t rsa` to generate, default exp/linux/exp.authorized_keys(empty file, must be modified before use)", default=os.path.join('exp','linux','exp.authorized_keys'))
